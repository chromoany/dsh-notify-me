  ...
  factory: (require) => {
    ...
    function applyTitle() {
      try {
        ...
        document.title = (config.enabled && attentionActive > 0)
          ? texts().marker + originalTitle
          : originalTitle;
      } catch (e) {}
    }
    ...
    // buttons); respects the master switch and permission state.
    ...
    function fireAttention(kind, sessionLabel, detail) {
      ...
      fire("attention", "DSH · " + label, (who ? who + d.sep : "") + content);
    }
    ...
    function evaluateRow(row) {
      ...
      if (newKeys.length > 0) {
        ...
        if (!detail) detail = "";
        fireAttention(it.kind, sessionLabelText(row), detail);
    ...
    var api = {
      ...
      setConfig: function (patch) { return setConfigLocal(patch); },
      ...
      test: function (kind) {
        ...
        unlockOnce();
        var d = texts();
        if (kind === "attention") fireAttention("question", "", d.testAttention);
        else fireDone("", d.testDone);
    ...
    var UI_en = {
      ...
      testHint: "Send one test alert with the current settings.",
    ...
    function NotifySettingsView(props) {
      ...
      function runTest(kind) {
        ...
        unlockOnce();
        if (kind === "attention") fireAttention("question", "", texts().testAttention);
        else fireDone("", texts().testDone);
