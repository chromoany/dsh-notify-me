...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function evaluateRow(row) {
      ...
      if (newKeys.length > 0) {
        var it = newKeys[0];
        fireAttention(it.kind, sessionLabelText(row), textFromPayload(it.payload));
    ...
    function bindUiSession() {
      if (uiSub) return;
      var svc = null;
      try { svc = rootCtx && rootCtx.uiSession; } catch (e) { svc = null; }
    ...
    function evalUiPending() {
      ...
      snap.forEach(function (it, sid) {
        ...
        fireAttention(it.kind, labelForSessionId(sid), textFromPayload(it));
        lastAttentionAt = Date.now();
        markAttention(key);
      });
