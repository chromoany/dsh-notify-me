...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // instead of just the app. Every failure here is non-fatal: a session that
    // left the list, an older host without `open()`, or a cordis service that
    // cannot be resolved must still leave a usable window in front.
    function focusSession(sessionId) {
      ...
      try {
        ...
        if (st && st.byId && !st.byId[sessionId] && st.current !== sessionId) return;
        svc.open(sessionId);
      } catch (e) {}
    }
    ...
    function deliver(kind, title, body, sessionId) {
      ...
      try {
        if (window.__dshNotifyMe && window.__dshNotifyMe.onEvent) {
          window.__dshNotifyMe.onEvent(kind, { title: title, body: body, sessionId: sessionId || null });
        }
      } catch (e) {}
    }
    function fire(kind, title, body, isCurrent, sessionId) {
      ...
      deliver(kind, title, body, sessionId);
      return true;
    }
    ...
    function flushQuieted() {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        fire("attention", copy.title, copy.body, false, copy.sessionId);
      }
    }
    ...
    function fireAttention(kind, sessionLabel, detail, isCurrent, sessionId) {
      var d = texts();
      ...
      if (quietedByVisibleSession(isCurrent)) return { title: title, body: body, sessionId: sessionId || null };
      fire("attention", title, body, isCurrent, sessionId);
      return null;
    }
    ...
    function evaluateRow(row) {
      ...
      if (newKeys.length > 0) {
        var it = newKeys[0];
        var copy = fireAttention(it.kind, sessionLabelText(row), textFromPayload(it.payload), !!row.face, row.id);
    ...
    function evalUiPending() {
      ...
      snap.forEach(function (it, sid) {
        ...
        var copy = fireAttention(it.kind, labelForSessionId(sid), textFromPayload(it), currentSessionId() === sid, sid);
